# 论文阅读 2024 — Xiaoxiao Zheng（AI 填写版）

> ⚠️ 本文件由 Claude 依据 PDF **文字层 + 逐页渲染图**填写。这次 Table I / II / III 和 Fig. 2–19 全部看到了，没有「图未提取」的格子；剩下的空缺都是**论文本身没说**，标为「未说明」。
...
- 最高增益下：BW **165 MHz**、输入折合噪声 **29.4 nA_rms**、相邻通道 SCR **−33.9 dB**
...
- 它自己给的定性话术：[5] 只有 ±125 μA 且噪声/功耗差；[22] 只消一个方向的直流、限制 PD 类型；[4][10][27][29] 都是单通道；[15] 带宽只有 1 MHz；[28] 噪声和摆幅差；[29] 噪声差。**它的组合牌是「FMCW + 双极性直流消除 + 多通道 + 线性输出」四个条件同时满足，表里没有第二家。**
...
1. **[14][15]（PD 和 AFE 之间串隔直电容）**：低截止频率需要大电容，**集成不了**；而且 PD 一端浮空接电容，电容充电过程中 **PD 结偏压跟着变 → 响应非线性**。→ 它把去直流挪进反馈环，PD 偏压被钳住（<10 mV 变化）。
...
1. **功耗**：51.48 mW/ch，20 通道整片 **≈ 1.03 W**【我算的】。多通道同行里只比 [22]（92 mW）好，比 [15] 7.8 mW、[28] 29.8 mW 差得多。其中 **输出缓冲占 38%**（19.8 mW，为了驱动片外 50 Ω 负载）——这一项是测试环境的账，不是结构的账；但核心 DCSF-TIA 也要 21.1 mW（两级各 3.2 mA）。
...
答：
> FMCW 雷达的接收管上永远流着一股很大的「底电流」——那是本振激光自己照出来的，不带任何目标信息，但它比回波信号大几百倍，会把高增益放大器直接顶饱和。这块芯片在放大器旁边加了一个**很慢的「自动配平器」**：它盯着放大器输出的平均值，往输入端反向灌一股一模一样的直流把底电流抵掉，放大器就只看得见来回抖动的那部分（拍频信号）。因为配平器动作很慢（大电容、小跨导），它只追得上直流和几十 kHz 以下的慢变化，追不上信号，所以信号不受影响。一条配平器负责往里灌电流，另一条负责往外抽，正负两种底电流都能配平。
...
我猜：TIA 单级 $Z_T\approx R_F$（式 9）；整链 $Z_T = R_F\times(R_{f0}/R_S)\times A_{buffer}$。五档由「多套 $R_F\|C_F$ 并联切换」+ 后级「$R_{f0}/C_{f0}$ 可变」共同实现。加了 DCL 后，**$z_0$ 到 $p_0$ 之间的低频跨阻低于 $R_F$，随频率上升逐渐回到 $R_F$**。
...
2. 🚩 **式 (24) 里没有 M3 的沟道噪声。** 第三项的 $1/g_{m8}$ 是 OTA 输入对的噪声电压折算，M3（真正往输入节点灌 250 μA 的那个 PMOS）自己的 $4kT\gamma g_{m3}$ 没出现。【我算的】M3 流 250 μA 时 $g_{m3}\approx1$–2 mS → **5 pA/√Hz 量级**，比整片宣称的 2.29 还大一倍。再加被消除的直流本身的散粒噪声 $\sqrt{2qI}$：100 μA → **5.7 pA/√Hz**，250 μA → **9 pA/√Hz**。
3. → **2.29 pA/√Hz 只在「DCL 空转（无直流输入）」时成立**（Q26b）。真实 FMCW 工况下噪底至少翻倍到三倍，而且是被消除的那股直流自己带来的，跟电路设计无关。**论文对此一个字没提。**
...
⚠️ **这一步我做不了，也不该替你做。** 把该标的三处内容准备好，你抄 Fig. 4 + Fig. 5 的时候往上填：
...
答：**把 FMCW 接收前端「去本振直流」这件事从 PD 前面集成不了的隔直电容，挪进 TIA 的反馈环里——用一个慢速 $g_m$-C 积分器合成出一个跟 $R_F$ 并联的大电感，直流短路、信号不管，两条镜像的环分别管灌和抽，于是 −150 ~ +250 μA 双极性直流被消掉、输入节点被钳住、PD 偏压不变；再配一个「OTA + 全差分 SF-TIA」后级把增益和输出共模解耦，拿到 1700 mV 线性摆幅。20 通道、0.18 μm、107 dB / 165 MHz / 29.4 nA_rms。**
...
答：
1. **真实工况下的噪声没有表征。** 2.29 pA/√Hz 是电学测试、（推测）零直流输入下的数。一旦 DCL 真的在消 100–250 μA，被消除电流的散粒噪声（5.7–9 pA/√Hz【我算的】）和 M3 电流源的沟道噪声（~5 pA/√Hz【我算的】）都进来，而式 (24) 里连 M3 那一项都没有。**这不是它做得不好，是所有「直流消除 / 背景光消除」方案的共同物理代价，但它没量化。**
2. **低频角随直流电流漂，且直流处只压了 8 dB。** $p_0\propto g_{m3}\propto I_{dc}$（亚阈值到饱和区跨两个数量级），所以「~20 kHz」是在某一个（未说明的）直流下测的；Fig. 15(b) 在 1 kHz 处仍有 99 dB，意味着零直流附近 DCL 环路增益只有 ~2.5 倍。近距离目标的拍频落在 99 → 107 dB 那段斜坡上，**强度信息不可信，且随目标远近变**。FMCW 要「同时拿距离和速度」，速度靠拍频差、距离靠拍频和，斜坡上的幅度误差会污染 FFT 峰值判决。
3. （附）**没有自动 AGC**，五档靠 SPI；**光学验证只有 20 MHz 一个点**，没有真正的 FMCW 测距演示；**通道间失配一个数都没有**（Fig. 17 标题写「20-channel」但只画一条曲线）。
...
3. ❓ 第一级「NMOS + PMOS 都做输入管」——这跟反相器 TIA 的区别只在多了尾电流源和共源共栅。**它说 PSRR 更好，代价是 headroom 少一个 $V_{ds,sat}$**。3.3 V 工艺无所谓，1.8 V 就得掂量。
4. ❓ 两条 DCL「按输入直流极性自动激活一条」——**怎么自动的？** Fig. 4 里没有选择逻辑，Fig. 5 只画了 SFP。猜是两条环同时开着、靠 M3（PMOS）和 STG 的 NMOS 哪个进导通区自然决定，那零直流附近两条都在亚阈值、都半开——这就是 Fig. 15(b) 那 8 dB 的来源？
...
7. ❓ Table III 里别人的 pA/√Hz 是实测密度还是也是 nA_rms/√f 换算的？[10] 1.44、[27] 0.84 这么低，是不是也是砖墙换算？**跨论文比噪声之前先做一次 B4。**

*系统一侧*

8. ❓ FMCW 拍频与距离的换算它没给具体的 B、T。【我算的】按它说的量级（B = 300 MHz、T = 300 μs）拍频 ≈ 6.7 kHz/m，20 kHz 高通对应 3 m，80 kHz 对应 12 m。**「中距 20–40 m」刚好在斜坡上方，是巧合还是有意的？**
9. ❓ 平衡 PD 剩 5–10% 直流（几十到 100 μA），那 250 μA 的消除上限对应什么本振功率？它没算这笔账。
10. ❓ 光学测试用 10 mW 直流光经衰减器 + 调制器，**测试时 PD 的直流光电流是多少？** 如果不为零，Fig. 19 的 SFDR 48 dB 就是「DCL 工作中」的真实线性度——这反而是全文最有价值的一个数，但它没标。
...
- 🚩 **抄进总表时 A5 打星号「整链」。** 跟 2021 那篇（也是整链 86 dBΩ）可以比，跟 Ma 2024 的 C-TIA 单级不能直接比。
...
- 🚩 **A13 填「线性度限定（±10%），≈6.8σ」并打星号。** 跟 2021 那篇的「2 μA、$d$ = 24」不是一个判据。

---

**Q26b. 噪声是在什么直流 / 背景条件下测的？**

答：**未说明。** Fig. 12(a) 电学测试台：$R_e$ 转压为流 + 50 Ω 端接 + 板上 $C_d$；噪声测试时输入大概率是端接到地、零直流。→ DCL 两条环都在亚阈值空转，M3 的噪声和散粒噪声都不在 29.4 nA 里（Q13 差在-2/3）。**这是本篇最该被追问的一个口径。**
...
答：这次图表全部看到了，**没有「没读到」的部分，只有「论文没写」的部分**：
1. $R_F$、$C_F$、$A_o$、$\omega_o$、$R_{f0}$、$R_S$、$C_S$ 的数值——**全篇一个都没给**，Q11/Q12/Q14 的数全是我从 Table I 反推的。
2. 测噪声和测低频响应时的直流输入电流。
3. 通道间失配、PVT 散布。
4. FMCW 系统参数（B、T、本振功率）。

**→ 请你补的三件事（按价值排序）：**
- **① 自己对着 Fig. 15(b) 读一次 −3 dB 点**（我读 80–100 kHz，作者说 20 kHz），并读 1 kHz 处的增益（我读 99.2 dB）。这决定 Q22-2 成不成立。
- **② 对着 Fig. 16 读五档的起点和终点**（我读 0.18–8 / 0.8–40 / 2–100 / 5–150 / 10–380 μA），算单档 DR。
- **③ Q16 手抄。** DCL 那条环画出来你就知道 $C_p$ 为什么是「不能去的那个器件」。
...
| A6 | 带宽 BW (−3dB) | **165 MHz @107 dB**；200/200/225/220 其他档 | 电学，板上 2 pF；低端高通 ~20 kHz（作者）/ ~80 kHz（我读图） |
...
> **⚠️ 最后提醒**：这份是 AI 填的。**Q16（手抄电路图）我故意留空**——DCL 那条环画一遍你才会真正记住 $C_p$ 在哪、$g_{m3}$ 和 $g_{m8}$ 各干什么。Q27 列的三件事（Fig. 15(b) 的 −3 dB 点、Fig. 16 五档端点、手抄）请你亲自做，前两件正好是本文口径矛盾的判定点。
