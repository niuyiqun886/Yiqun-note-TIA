...
$$
n = (\frac{14.6}{10})^2 = 2.13 \to 3次, \,\,\, t_{meas} = \frac{3}{100kHz} = 30 \mu s
$$

校核二： 行走误差(决定要不要换用双阈值)

单阈值，固定 $V_T=26.7 ，t_p = \frac{V_T}{V_{peak}}$ ：

| 输入     | $V_{peak}$ | $t_p$​  |
| ------ | ---------- | ------- |
| 358 nA | 35.8 mV    | 5.34 ns |
| 10 µA  | 1000 mV    | 0.19 ns |

$$
\Delta t_{walk} = 5.15\ \text{ns} \quad\Rightarrow\quad \Delta R = 77\ \text{cm}
$$
比 10 cm 精度大一个数量级，**必须用双阈值校正**：

$$
t_0 = t_1 - \frac{\Delta t_d}{C-1}​​
$$
...
|---|---|---|
|带宽|60 MHz|脉冲上升时间|
|TIA 输入噪声|≤ 4.0 pA/√Hz|链路预算 + d=9.2d=9.2 d=9.2|
|最高跨阻增益|100 dBΩ|摆幅 / 单档饱和|
|增益档位数|4 档（100/80/60/46 dBΩ）|全量程 108 dB|
|输入阻抗|< 50 Ω|Cin=2C_{in}=2 Cin​=2 pF 分流|
|比较器门限|26.7 mV（高增益档）|k=6.87k=6.87 k=6.87|
|比较器数量|2（双阈值）|行走误差 77 cm|
|单次测量精度|14.6 cm|抖动|
|平均次数|3|精度补差|
